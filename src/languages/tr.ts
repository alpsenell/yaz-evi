export default {
  welcomeToYazEvi: "Yaz Evi",
  exclusiveHome: "Bozcaada'nın en özel evi",
  home: "Ana Sayfa",
  rooms: "Odalar",
  gallery: "Galeri",
  experiences: "Deneyimler",
  contact: "İletişim",
  selectOption: "Bir seçenek seçin",
  english: "İngilizce",
  turkish: "Türkçe",
  room1: "Oda Zeus",
  room2: "Oda Hera",
  room3: "Oda Achilles",
  room1Description: "Oda açıklaması",
  homepage: {
    untouchedNature: "El değmemiş doğa",
    textAndButton: {
      title: "Bozcaada'nın en özel konuk evi",
      descriptionFirst: 'Bozcaada, Ege Denizi\'nin masmavi sularında adeta bir huzur cenneti sunar. Bağların yeşiline, rüzgar güllerinin beyazına ve tertemiz sahillerin altın sarısına ev sahipliği yapan bu ada, doğasıyla büyüler. Yemyeşil tepelerden denize uzanan zeytin ağaçları ve çam ormanları, adanın her köşesine doğal bir serinlik getirir. Akvaryum kadar berrak koylarında yüzmek, insana doğanın kollarında yeniden doğmuş hissi verir. Gün batımında Polente Feneri’nden izlenen manzara, denizin altın rengindeki dalgalarla dansını sunar. Bozcaada, deniziyle ruhu dinlendirirken, doğasıyla da tüm yorgunlukları siler.',
      descriptionSecond: 'Bozcaada’nın doğal güzellikleri kadar, ada yaşamının sadeliği ve dinginliği de ziyaretçilerini kendine çeker. Daracık taş sokaklarında dolaşırken begonvillerle süslenmiş beyaz evlerin arasından yayılan kahve ve deniz kokusu, insana huzur verir. Gün boyu esen rüzgar, adayı sadece serinletmekle kalmaz, aynı zamanda yelkenlilere eşlik eden bir melodiyi taşır. Sahilde küçük balıkçı teknelerinin dingin salınımları, adanın zamanın yavaş aktığı ruhunu gözler önüne serer. Gün batımında lezzetli Ege mezeleriyle donatılmış bir sofrada oturmak, Bozcaada\'nın tadını çıkarmanın en keyifli yollarından biridir.',
      button: "Konaklamanızı Ayırtın"
    }
  },
  roomsPage: {
    title: 'Konforlu Konuk Evi',
    subtitle: 'Doğanın kucağında özel bir taş ev',
    paragraphs: {
      one: 'Bozcaada\'nın en özel lüks tatil yeri, huzurlu Ege Adası\'nda sizleri bekliyor. Doğanın muhteşem güzellikleriyle çevrili, modern lüksle geleneksel cazibeyi bir araya getiren özel villalarımızda konaklayın ve adanın sakin atmosferine kendinizi bırakın. Birinci sınıf olanaklar ve kusursuz hizmet anlayışıyla Bozcaada Exclusive Villas, huzurlu bir lüks deneyimini yeniden tanımlıyor.',
      two: 'Ana villa, zarif bir yemek odası ve açık plan mutfağın yanı sıra, el yapımı yerel dekorlarla döşenmiş rahat bir oturma alanı, Ege’nin en seçkin şaraplarıyla dolu özel bir mahzen, spa alanı ve parıldayan denizi kucaklayan sonsuzluk havuzuyla özenle seçilmiş imkanlar sunar. Bahçede taş fırında hazırlanan taze lezzetlerle açık hava yemeklerinin tadını çıkarın.',
      three: 'İç mekanlar, Bozcaada\'nın zengin mirasını yansıtan el yapımı tekstiller, yerel malzemelerden üretilmiş ahşap mobilyalar ve adanın huzur veren güzelliklerinden ilham alan benzersiz detaylarla tasarlanmıştır. Doğal taş duvarlar ve toprak tonlarındaki zeminler, sıcak ve davetkar bir atmosfer yaratarak Ege\'nin cazibesini yansıtır.',
      four: 'Mülk, dalgalanan üzüm bağlarının arasında, parıldayan Ege Denizi\'ne karşı konumlanmış olup, izole plajlara ve butik mağazalarla dolu taş sokaklara kolay erişim sağlar.',
      five: 'Dört özel villamızı keşfedin – Üzüm Bağı Manzarası, Sahil Sakinliği, Zeytin Bahçesi Saklanma Yeri ve Gün Batımı Cenneti.'
    },
    button: 'Fiyatlar ve Dahil Olanlar'
  }
}
